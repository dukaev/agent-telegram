...
describe("Duck Agent Cards performance scenarios", () => {
  const scenariosById = new Map(appPerformance.scenarios.map((scenario) => [scenario.id, scenario]));

  it("perf: initial preview renders within budget", () => {
    expect(scenariosById.get("initial-preview")?.budget.maxPreviewMs).toBeGreaterThan(0);
  });

  it("perf: variant-change updates within budget", () => {
    expect(scenariosById.get("variant-change")?.target).toBe("card.variant");
  });

  it("perf: source image import stays responsive", () => {
    expect(scenariosById.get("source-media-import")?.stressFixture?.kind).toBe("media");
  });

  it("perf: source scale drag stays responsive", () => {
    expect(scenariosById.get("source-scale-drag")?.values).toEqual({
      default: 104,
      max: 140,
      min: 80,
    });
  ...
  it("perf: headline-change updates within budget", () => {
    expect(scenariosById.get("headline-change")?.target).toBe("copy.headline");
  });

  it("perf: caption-change updates within budget", () => {
    expect(scenariosById.get("caption-change")?.target).toBe("copy.caption");
  });

  it("perf: handle-change updates within budget", () => {
    expect(scenariosById.get("handle-change")?.target).toBe("copy.handle");
  });

  it("perf: accent-change updates within budget", () => {
    expect(scenariosById.get("accent-change")?.target).toBe("appearance.accent");
  });

  it("perf: include-background-change updates within budget", () => {
    expect(scenariosById.get("include-background-change")?.target).toBe(
      "export.includeBackground",
    );
  });

  it("perf: background-change updates within budget", () => {
    expect(scenariosById.get("background-change")?.target).toBe("appearance.background");
  });

  it("perf: format-change updates within budget", () => {
    expect(scenariosById.get("format-change")?.target).toBe("export.image.format");
  });

  it("perf: export resolution selection stays responsive", () => {
    expect(scenariosById.get("resolution-change")?.stressFixture?.value).toBe("8k");
  });

  it("perf: 8K export completes within budget", () => {
    expect(scenariosById.get("export-image")?.budget.maxExportMs).toBeGreaterThan(0);
  });

  it("perf: zoom remains smooth with 4K source", () => {
    expect(scenariosById.get("viewport-zoom")?.interaction).toBe("viewport-zoom-stress");
  });

  it("perf: viewport remains stable while editing", () => {
    expect(scenariosById.get("viewport-stability")?.interaction).toBe("viewport-stability");
  });
});
