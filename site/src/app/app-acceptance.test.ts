...
describe("Duck Agent Cards product observables", () => {
  ...
  it("source media lifecycle reaches preview and export", () => {
    ...
    expect(appSchema.media.defaultAssets[0]?.sourceTarget).toBe("source.image");
  });

  it("image scale changes live source crop", () => {
    expect(acceptanceById.get("source.scale")?.evidence).toBe("product-output");
  });

  it("headline text reaches every layout", () => {
    expect(acceptanceById.get("copy.headline")?.target).toBe("copy.headline");
  });

  it("caption text reaches product output", () => {
    expect(acceptanceById.get("copy.caption")?.target).toBe("copy.caption");
  });

  it("telegram handle reaches product output", () => {
    expect(acceptanceById.get("copy.handle")?.target).toBe("copy.handle");
  });
  ...
  it("include background controls preview and export alpha", () => {
    expect(acceptanceById.get("export.includeBackground")?.expectedObservable).toMatch(
      /preview product background.*transparent PNG alpha/i,
    );
  });

  it("background color reaches preview and export", () => {
    expect(acceptanceById.get("appearance.background")?.expectedObservable).toMatch(
      /product background.*export/i,
    );
  });
  ...
  it("custom renderer exposes semantic product layers", () => {
    expect(acceptanceById.get("renderer.card")?.componentType).toBe("DOM product renderer");
  });
});
