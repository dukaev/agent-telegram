...
async function listenFrom(host, startPort) {
  ...
  throw new Error(`No test port available from ${startPort}.`);
}
...
test("findAvailablePort skips ports occupied on IPv6 localhost", async (t) => {
  ...
  try {
    server = await listenFrom("::1", 43_000);
  } catch (error) {
...
test("findAvailablePort skips ports occupied on IPv4 localhost", async (t) => {
  const server = await listenFrom("127.0.0.1", 44_000);
  t.after(() => close(server));
