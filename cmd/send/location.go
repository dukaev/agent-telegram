...
// LocationCmd represents the send-location command.
...
// AddLocationCommand adds the location command to the root command.
func AddLocationCommand(rootCmd *cobra.Command) {
	rootCmd.AddCommand(LocationCmd)

	LocationCmd.Flags().StringVar(&sendLocationCity, "city", "", "Search by city/place name instead of coordinates")

	LocationCmd.Run = func(_ *cobra.Command, args []string) {
	...
	sendLocationFlags.RegisterWithoutCaption(LocationCmd)
}
